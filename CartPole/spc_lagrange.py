...
# Single pendulum on a cart (dpc) simulation code
#
# This file generates the model from symbolic expressions and generates a
# file with the name `spc_dynamics_generated.py`
...
p = Matrix([r_1, m_c, m_1, g])      # parameter vector

q = Matrix([q_0, q_1])                   # generalized positions
qdot = Matrix([qdot_0, qdot_1])       # time derivative of q
qddot = Matrix([qddot_0, qddot_1])   # time derivative of qdot
...
lagrange_eq = partial_L_by_partial_q - d_inner_by_dt + Matrix([f,0]) #Matrix([f,0,0])
...
s = lambdastr((q_0, q_1, qdot_0, qdot_1, f, r_1, m_c, m_1, g),
              [qdot_0, qdot_1, qddot_0, qddot_1])

f_gen = open("spc_dynamics_generated.py", 'w')
